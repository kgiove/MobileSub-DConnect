 ...
 * @author ietservizi
 */
public class VisitSale extends Visit {
    
    public void start(){
        try {
            setTypeSku(0);
            setTitleForm(LangXml.Sale_name);
            open_AllRecords();
            create_HashTables();
            load_HashTables_product();
            if(ActionProductMaster.addAllControlSales == 0)
                list_Skus();
            else
                list_addAllSkus();
        }catch(Exception e) {}
    ...
    public String retrieveAllSkusTable(){
        return "SALE;"+super.retrieveAllSkusTable();
    }
        ...
        if(d.getId()==1 ){
            ...
            if(numTel!=null && !numTel.equals("")){
                switch(result){
                    ...
                    break;
                    case 0: if(sendThrowsGPRS(numTel,curr_PMVersion+msg)){
                    ...
                    break;
                    case 1:if(sendThrowsGPRS(numTel,curr_PMVersion+msg)){
                        ActionProductMaster.addAllControlSales = 0;
                        save_SkusReport(0);
                        list_Skus();
                    }else{
